...
export default {
  ...
  setup() {
    ...
    const currentXP = computed(() => {
      ...
      return progress.value.totalXp % nextLevelXP;
    });
      ...
      authService.onAuthStateChanged(async (firebaseUser) => {
        if (firebaseUser) {
          try {
            ...
            // Carrega dados SEM usar store - usa services diretamente
            const userService = await import('../services/user.service');
            
            const userInfo = await userService.default.getUser(firebaseUser.uid);
            const userProgressData = await userService.default.getUserProgress(firebaseUser.uid);
